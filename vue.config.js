...
function getCdnPath(env) {
  console.log('cdn')
  const prodCdnHost = 'https://static.youcash.com/pages';
  const grayCdnHost = 'https://static.youcash.com/pages';
  let assetsPath = '';
  if (IS_PROD) {
    assetsPath = prodCdnHost + '/' + OUTPUT_DIR + './';
  }
  if (process.env.NODE_ENV === 'gray') {
    assetsPath = grayCdnHost + '/' + OUTPUT_DIR + './';
  }
...
const cndPath = getCdnPath(env);
module.exports = {
    // publicPath:BASE_URL,//publicPath取代了baseUrl
    // publicPath:'./',
    publicPath:(process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'gray') ? cndPath : './',//publicPath取代了baseUrl
    ...
    devServer:{
        ...
        proxy: {
          '/api': {
            target: 'https://alissl.youcash.com', // 接口的域名
